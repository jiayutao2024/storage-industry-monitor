(() => {
  ...
  function cycle(D,R) {
    ...
    const indexedGroup=product=>{const rows=hist.filter(x=>x.product===product),ids=[...new Set(rows.map(x=>x.metric_id))],cats=[...new Set(rows.map(x=>x.date))].sort();const series=ids.map((id,i)=>{const one=rows.filter(x=>x.metric_id===id).sort((a,b)=>a.date.localeCompare(b.date));if(one.length<2)return null;const base=Number(one[0].price),map=Object.fromEntries(one.map(x=>[x.date,Number(x.price)/base*100]));return {name:one[0].segment||id,values:cats.map(c=>map[c]??null),color:COLORS[i%COLORS.length]}}).filter(Boolean);return {cats,series}};
    const dram=indexedGroup("DRAM"),nand=indexedGroup("NAND"),changes=metrics.filter(x=>finite(x.change_pct));
    ...
    const body=`<section class="section"><div class="callout"><b>价格口径已重做：</b>绝对价格只在同一规格内比较；跨规格图统一转为“首个有效观察点=100”的指数。缺失值不再补0，变化为0仅表示来源页当期确实未变。</div></section><section class="section storage-grid-2">${chartCard("DRAM公开价格指数","同规格首个有效点=100；不混合颗粒、模组的绝对价格",dram.series.length?lineChart("dram-price-index",dram.cats,dram.series,{digits:1}):'<div class="empty">至少积累两个有效观察点后展示趋势</div>',`自建公开快照 · ${dram.cats.length}个观察日`)}${chartCard("NAND公开价格指数","同规格首个有效点=100；不把不同容量价格直接相加",nand.series.length?lineChart("nand-price-index",nand.cats,nand.series,{digits:1}):'<div class="empty">至少积累两个有效观察点后展示趋势</div>',`自建公开快照 · ${nand.cats.length}个观察日`)}</section><section class="section storage-grid-2">${chartCard("最新有效价格变化","仅展示来源明确且价格>0的规格；0%保留为真实横盘",barChart("price-change",changes.map(x=>x.segment),changes.map(x=>Number(x.change_pct)),{percent:true}),"变化周期遵循来源页") }<article class="card storage-card"><h3>历史深度审计</h3>${table(["规格","有效点","起始","最新"],depth.map(x=>`<tr><td>${x.name}</td><td class="num">${x.count}</td><td>${x.from}</td><td>${x.to}</td></tr>`))}<p class="metric-foot">少于8个观察点的序列只作价格锚，不据此判断完整周期形态。</p></article></section><section class="section card storage-card"><h3>价格明细与来源</h3>${table(["产品","规格","报价类型","均价","变化","数据日","来源"],metrics.map(x=>`<tr><td><span class="tag red">${x.product}</span></td><td><b>${x.item}</b><br><small>${x.segment}</small></td><td>${x.quote_type}</td><td class="num">$${num(x.price,3)}</td><td class="num">${finite(x.change_pct)?(Number(x.change_pct)>0?"+":"")+num(x.change_pct,2)+"%":"—"}</td><td>${(x.observed_at||"").slice(0,16).replace("T"," ")}</td><td><a href="${x.source_url}" target="_blank" rel="noopener">${x.source_name||"来源"} ↗</a></td></tr>`))}<p class="source-note">单位遵循来源页；颗粒、模组、晶圆与整盘不可直接比较绝对价格。</p></section>`;
    return {html:shell("cycle",body),charts:{"dram-price-index":{categories:dram.cats,series:dram.series,unit:"",digits:1,note:"各规格首个有效观察点=100"},"nand-price-index":{categories:nand.cats,series:nand.series,unit:"",digits:1,note:"各规格首个有效观察点=100"},"price-change":{categories:changes.map(x=>x.item),values:changes.map(x=>Number(x.change_pct)),name:"变化",unit:"%",digits:2}}};
  }
  ...
  function companies(D,R) {
    ...
    const stockSeries=core.map((symbol,i)=>{const rows=mh.filter(x=>x.symbol===symbol&&stockDates.includes(x.trade_date)).sort((a,b)=>a.trade_date.localeCompare(b.trade_date));if(rows.length<2)return null;const base=Number(rows[0].price),map=Object.fromEntries(rows.map(x=>[x.trade_date,Number(x.price)/base*100]));return {name:rows[0].short_name||rows[0].name||symbol,values:stockDates.map(d=>map[d]??null),color:COLORS[i]}}).filter(Boolean);
